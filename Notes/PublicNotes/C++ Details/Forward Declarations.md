...
**NOTE**: memory sizes aren't always this easy to determine, memory alignment issues could complicate it, so don't treat the numbers in these examples as facts.
...
In 64-bit mode, a pointer is 64 bits (or 8 bytes) big. So, our GameObject is 20 bytes large (See **NOTE** at the top, its actually 24 bytes)
...
Now, if we need to use the Mesh class by accessing its methods or member vars, the compiler needs to know everything about it, so an include is required.
