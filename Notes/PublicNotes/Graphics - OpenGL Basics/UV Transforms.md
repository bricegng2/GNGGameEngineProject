...
This is definitely optional, and could be considered a bad idea because of rotation, but we can fit 4 number in a single vec4. This would mean only passing one uniform into the vertex shader instead of 2. We could call this a UV Transform instead, but keep in mind it doesn't handle rotation, for rotation we'd need a 3x2 matrix of values.
