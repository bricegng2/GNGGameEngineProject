...
![CoordinatePlane_ObjectSpace](../Images/CoordinatePlane_ObjectSpace.png)
...
![CoordinatePlane_WorldSpace](../Images/CoordinatePlane_WorldSpace.png)
...
![CoordinatePlane_ViewSpace](../Images/CoordinatePlane_ViewSpace.png)
...
Without a scale applied, we would only see the region of view space that extends from -1 to 1, i.e. a small empty area in the center of that space.

In this example, we want to display a 20x20 region of the world centered on the camera, so we simply multiple all coordinates by 1/10.
We choose 1/10, so points at "10" will become "1", and points at "-10" will become "-1". This gives as a total viewing area of 20 units horizontally and vertically, which is good for a square window. Keep reading for dealing with aspect ratios.

![CoordinatePlane_ClipSpace](../Images/CoordinatePlane_ClipSpace.png)

## The Example Above in Code

All of this coordinate system manipulation happens in the vertex shader.
...
This is without aspect ratio correction, the image on the right ends up stretching out to fill our window and circle shapes are no longer circles.
