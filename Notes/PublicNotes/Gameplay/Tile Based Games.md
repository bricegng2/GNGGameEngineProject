...
##### Terrible Idea: Each tile as its own object.
...
This may work for small tilemaps, but a lot of data ends up being duplicated, making it use up a lot more ram, and makes it harder to manage and find bugs. For example, it would be possible for all walls to block you, but one has its "walkable" bool set to true by mistake.
...
Since the TileType enum is *scoped*, we can't use its values as numbers directly, so we need to typecast to int when we want to use them.
