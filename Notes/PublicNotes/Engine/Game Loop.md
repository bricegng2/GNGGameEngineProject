...
Waiting for the monitor to finish presenting the framebuffer contents is called vertical syncing (V-Sync). The video driver can send a signal, called a v-blank, to let the application know it's finished displaying the image and your game can then swap the buffers and present the next image when its ready.
...
Enabling/disabled vertical sync is not handled by OpenGL, it's handled by the operating system, so its a different mechanism for each platform you want to support, whether that's iOS, Android, Windows, OSX, Linux or any of the consoles.
