...
```c++
	...
	// Alternative, potentially a bit slower since it does the lookup twice
	if( m_StudentGrades.count("Fred") > 0 )
	{
		int grade = m_StudentGrades["Fred"];
	}
```
...
Iterating will return a "pair" with matching template arguments as the map
The key and value of the pair can be accessed using "first" and "second" as seen here:

```c++
for( const std::pair<std::string, float>& studentGradePair : m_StudentGrades )
{
	std::string name = studentGradePair.first;
	float grade = studentGradePair.second;
}
```

You'll probably want to use auto instead for cases like this:

```c++
for( const auto& studentGradePair : m_StudentGrades )
{
	std::string name = studentGradePair.first;
	float grade = studentGradePair.second;
}
