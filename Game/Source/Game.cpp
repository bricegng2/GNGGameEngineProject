...
Game::Game(fw::FWCore& fwCore)
    : GameCore(fwCore)
    , onCharEventHandler(std::bind(&Game::onCharEvent, this, std::placeholders::_1))
{
    runNetworkTestCode();
    ...
    m_pEventManager->registerListenerFunction<fw::CharEvent>(&onCharEventHandler);
    ...
    //m_pActiveScene = new PhysicsScene( this );
    //m_pActiveScene = new Physics3DScene( this );
    m_pActiveScene = new IslandScene(this);
}
...
void Game::update(float deltaTime)
{
    ...
    {
        if (ImGui::Checkbox(" ", &m_isWireframeOn))
        {
            if (m_isWireframeOn)
            {
...
void Game::draw()
{
	...
	vec4 diffuseColor = vec4(1, 0, 1, 1);
    m_pUniforms->set("u_diffuseColor", diffuseColor);
