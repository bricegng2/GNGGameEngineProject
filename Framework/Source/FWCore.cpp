...
namespace fw
{
    ...
    bool FWCore::createRenderWindow(char* title, uint32 width, uint32 height, uint8 colorBits, bool fullscreenflag)
    {
        ...
        // Create our window.
        {
            ...
            rid.dwFlags = 0; // Or RIDEV_NOLEGACY to block legacy mouse input
            rid.hwndTarget = m_hWnd;
            if (!RegisterRawInputDevices(&rid, 1, sizeof(rid)))
            {
                OutputDebugString("Failed to register raw input devices.\n");
            }
        }
    ...
    LRESULT CALLBACK FWCore::wndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        case WM_MOUSEMOVE:
            {
                int currentMousePosX = GET_X_LPARAM( lParam );
                int currentMousePosY = GET_Y_LPARAM( lParam );
        
                ////if (pFWCore->isMouseButtonDown(1))
                //{
                //    MouseEvent* pEvent = new MouseEvent(
        ...
        case WM_INPUT:
        {
            ...
            if (GetRawInputData((HRAWINPUT)lParam, RID_INPUT, lpb, &dwSize, sizeof(RAWINPUTHEADER)) == dwSize)
            {
                ...
                if (raw->header.dwType == RIM_TYPEMOUSE)
                {
                    ...
                    MouseEvent* pEvent = new MouseEvent(x, y);
