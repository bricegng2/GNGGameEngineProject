...
namespace fw
{
    ...
    void PhysicsBodyJolt::setLinearDamping(float linearDamping)
    {
        m_pBody->GetMotionProperties()->SetLinearDamping( linearDamping );
    }

    void PhysicsBodyJolt::setAngularDamping(float angularDamping)
    {
        m_pBody->GetMotionProperties()->SetAngularDamping( angularDamping );
    }

    void PhysicsBodyJolt::limitTo2D()
    {
        JPH::BodyCreationSettings settings = m_pBody->GetBodyCreationSettings();
        m_pBody->GetMotionProperties()->SetMassProperties( JPH::EAllowedDOFs::Plane2D, settings.GetMassProperties() );
    }
    ...
    void PhysicsBodyJolt::applyForceAtWorldPos(vec3 force, vec3 worldPoint)
    {
        m_pBody->AddForce( JPH::RVec3(force.x, force.y, force.z), JPH::RVec3(worldPoint.x, worldPoint.y, worldPoint.z) );
    }

    void PhysicsBodyJolt::editor_fillInspectorWindow()
    {
    }
