...
fw::Mesh* createD6Mesh(float width, float height, float depth)
{
    ...
    int vertBytes = sizeof(VertexFormat_PosUV) * (int)m_verts.size();
    return new fw::Mesh( VertexFormat_PosUV::format, m_verts.data(), vertBytes, g_D6Indices, sizeof(g_D6Indices) );
}
