...
set shadercompiler=..\..\tools\shaderc
...
@FOR %%G in ("Shaders\*.vert") DO (
    %shadercompiler% -f %%G -o ../Data/Shaders/dx11/%%~nxG.bin  --type v --platform windows -i ../../Libraries/bgfx/src -p s_5_0
...
@FOR %%G in ("Shaders\*.frag") DO (
    %shadercompiler% -f %%G -o ../Data/Shaders/dx11/%%~nxG.bin  --type f --platform windows -i ../../Libraries/bgfx/src -p s_5_0
    IF !ERRORLEVEL! NEQ 0 (echo Errors in %%G && goto error) ELSE (echo Built dx11 %%G)
