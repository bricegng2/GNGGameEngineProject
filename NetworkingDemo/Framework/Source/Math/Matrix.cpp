...
namespace fw
{
    ...
    void mat4::createSRT(float sc, vec3 rot, vec3 pos)
    {
        setIdentity();
        scale(sc);
    ...
    void mat4::createSRT(vec3 sc, vec3 rot, vec3 pos)
    {
        createScale(sc.x, sc.y, sc.z);
        rotate(rot.z, 0, 0, 1); // roll
        rotate(rot.x, 1, 0, 0); // pitch
        rotate(rot.y, 0, 1, 0); // yaw
        translate(pos.x, pos.y, pos.z);
    ...
    void mat4::scale(float sc)
    {
        m11 *= sc; m21 *= sc; m31 *= sc; m41 *= sc;
        m12 *= sc; m22 *= sc; m32 *= sc; m42 *= sc;
        m13 *= sc; m23 *= sc; m33 *= sc; m43 *= sc;
    }
    ...
    void mat4::scale(vec3 sc)
    {
        m11 *= sc.x; m21 *= sc.x; m31 *= sc.x; m41 *= sc.x;
        m12 *= sc.y; m22 *= sc.y; m32 *= sc.y; m42 *= sc.y;
        m13 *= sc.z; m32 *= sc.z; m33 *= sc.z; m43 *= sc.z;
    }
